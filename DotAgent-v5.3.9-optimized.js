...
function cleanupPriorityQueue(sequence) {
    ...
    } catch (e) { console.error("Ignored exception:", e); }
}
    ...
    ui.run(function() {
        if (appState.masks && appState.masks.length > 0) {
            appState.masks.forEach(w => {
                try { w.close(); } catch (e) { console.error("Ignored exception:", e); }
            });
            appState.masks = [];
            log("已清理所有视觉遮罩");
        }
...
function runSingleMonitorThread(sequence, sequenceKey) {
    ...
    let monitorThread = threads.start(function () {
        ...
        while (!threads.currentThread().isInterrupted()) {
            ...
            // (PQ 维护逻辑保持不变)
            try {
                try { cleanupPriorityQueue(sequence); } catch (e) { console.error("Ignored exception:", e); }
            ...
            try {
                        ...
                        if (trigger.type === 'image') {
                            ...
                            if (!ocrTarget) {
                                ...
                                if (searchRegion) {
                                    ...
                                    } catch (e) { console.error("Ignored exception:", e); } finally { if (subImg) subImg.recycle(); }
                                } else {
                                    // 全屏识别 (直接跑)
                ...
                if (new Date().getTime() % 30000 < interval) {
                    /* java.lang.System.gc() removed for performance */
                }
        ...
        for (let key in threadImageCache) {
            try { if (threadImageCache[key]) threadImageCache[key].recycle(); } catch (e) { console.error("Ignored exception:", e); }
        }
        threadImageCache = {};
    });
