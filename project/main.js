...
// --- 引用常量 ---
const CONSTANTS = Config.GLOBAL_CONSTANTS;
const GLOBAL_DEFAULTS = Config.DEFAULT_SETTINGS;
__PQ_DEBUG = false;
...
function getTriggerId(trigger) {
    try {
        ...
        const areaHash = Utils.stableHash(trigger.search_area || trigger.area || null);
...
function bumpTriggerPriority(sequence, trigger) {
    ...
    try {
        ...
        try { PQManager.write(sequence, currentProfileName); } catch (e) { }
    } catch (e) { }
...
let appSettings = JSON.parse(JSON.stringify(GLOBAL_DEFAULTS));
let sequences = {};
...
GLOBAL_DEFAULTS.yOffset = statusBarHeight; // 自动将默认偏移设为状态栏高度
...
ui.saveGraphicalSettingsBtn.click(() => {
    try {
        ...
        // --- 2. 修改下面的 if 语句 ---
        if (!Utils.validateNumericInput(clickDelayStr) || !Utils.validateNumericInput(swipeDurationStr) || !Utils.validateNumericInput(yOffsetStr) || !Utils.validateNumericInput(panelWidthStr) || !Utils.validateNumericInput(targetViewSizeStr) || !Utils.validateNumericInput(defaultCachePaddingStr)) {
            return;
        }
...
function handleOcrSuccess(ocrResult, successAction) {
    ...
    switch (successAction.action) {
        case 'click':
            ...
            Utils.safePress(clickX, clickY, CONSTANTS.UI.CLICK_PRESS_DURATION_MS);
    ...
    for (let i = 0; i < tasksToRun.length; i++) {
        ...
        switch (task.type) {
            ...
            case 'ocr': {
                ...
                if (task.cachedBounds && task.cachedBounds.left !== undefined) {
                    ...
                    if (captured) {
                        ...
                        var region = Utils.calculatePaddedRegion(b, padding);
            ...
            case 'wait_for_dissapear': {
                ...
                if (task.search_area && task.search_area.length === 4) {
                    // --- 核心修复：使用 Utils.calculatePaddedRegion 来限制 search_area ---
                    let [x1, y1, x2, y2] = task.search_area;
                    let searchBounds = { left: x1, top: y1, right: x2, bottom: y2 };
                    findOptions.region = Utils.calculatePaddedRegion(searchBounds, 0); // 0 padding
                    // --- 修复结束 ---
                }
...
function runSingleMonitorThread(sequence, sequenceKey) {
    ...
    let monitorThread = threads.start(function () {
        ...
        let __triggersSig = Utils.stableHash(sequence.triggers || []);
        ...
        while (!threads.currentThread().isInterrupted()) {
            ...
            const __curSig = Utils.stableHash(sequence.triggers || []);
            ...
            try {
                ...
                var __pqObj = PQManager.tryLoad(sequence, currentProfileName);
                    ...
                    if (trigger.type === 'image') {
                        ...
                        if (!ocrTarget) {
                            ...
                            if (trigger.search_area && trigger.search_area.length === 4) {
                                ...
                                let [x1, y1, x2, y2] = trigger.search_area;
                                let searchBounds = { left: x1, top: y1, right: x2, bottom: y2 };
                                ocrOptions.region = Utils.calculatePaddedRegion(searchBounds, 0); // 0 padding
                                // --- 修复结束 ---
                            }
...
function createTargetView() {
    ...
    try {
        ...
        uiRefs.targetView.root.setBackgroundColor(android.graphics.Color.parseColor(GLOBAL_DEFAULTS.theme.targetViewColor));
    }
...
function createTaskWindow(task, sequence) {
    ...
    let color;
    try { color = android.graphics.Color.parseColor(appSettings.theme.taskClickColor); } catch (e) { color = android.graphics.Color.parseColor(GLOBAL_DEFAULTS.theme.taskClickColor); }
...
function createSwipeVisuals(task, sequence) {
    ...
    function createMarker(text, x, y, onClickCallback) {
        ...
        let color;
        try { color = android.graphics.Color.parseColor(visual.originalBg); } catch (e) { color = android.graphics.Color.parseColor(GLOBAL_DEFAULTS.theme.taskSwipeColor); }
        win.root.setBackgroundColor(color);
...
function addWaitTask(targetSequence, onComplete) {
    dialogs.rawInput("输入等待时间 (毫秒)", "1000").then(durationStr => {
        ...
        if (!Utils.validateNumericInput(durationStr)) return;
    ...
    ui.post(() => {
        ...
        view.pq_saveBtn.click(() => {
            try {
                ...
                if (Utils.validateNumericInput(newMaxLengthStr)) {
...
function populateGraphicalSettings() {
    if (ui.clickDelayInput) {
        ui.run(() => {
            ...
            if (!appSettings.theme) { appSettings.theme = JSON.parse(JSON.stringify(GLOBAL_DEFAULTS.theme)); }
...
function loadProfile(profileName) {
    ...
    if (files.exists(profilePath)) {
        try {
            ...
            let finalSettings = JSON.parse(JSON.stringify(GLOBAL_DEFAULTS));
...
function resetToDefaultProfile() {
    ...
    appSettings = JSON.parse(JSON.stringify(GLOBAL_DEFAULTS));
    sequences = {
        "main": {
            name: "主序列",
        ...
        ui.run(() => {
            ...
            newProfileView.click(() => {
                dialogs.rawInput("输入新方案名称", "my_profile").then(name => {
                    ...
                    const emptyProfile = { version: CONSTANTS.VERSION, settings: GLOBAL_DEFAULTS, sequences: {} };
                ...
                dialogs.rawInput(`请输入位于 "${backupDirName}" 文件夹中的配置文件名`, "export_default.json").then(fileName => {
                    ...
                    if (files.exists(path)) {
                        try {
                            ...
                            let newSettings = JSON.parse(JSON.stringify(GLOBAL_DEFAULTS));
